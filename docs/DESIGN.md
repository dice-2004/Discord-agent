...
### 0.3. 実装対象の統一方針（フェーズ分割しない）

本仕様では、機能をフェーズで除外しません。Research Agent / Eternal Explorer を含め、すべてを**設計対象**として扱います。

ただしN100保護のため、以下はデフォルトで「常時起動しない」運用とします。

* 長時間常駐ジョブ（永久探索ループ等）
* Web UI、管理画面、REST APIサーバー
* 追加の常駐ワーカー
* 大規模バッチ取り込み（カスタムドキュメント全文再インデックス等）

上記も実装対象から外すのではなく、**起動条件・キュー制御・チェックポイント設計を持った拡張ポイントとして維持**すること。
...
### 0.5. 本運用の必須4要件（2026-04-01追記）

以下4点は本運用判定の必須条件とする。

1. フェーズ名による除外を行わない。未実装機能は「未実装エラー」を返し、疑似成功を禁止する。
2. 重い処理はキューに通し、同時実行数を `MAX_CONCURRENT_HEAVY_TASKS` で制限する。
3. 長時間タスクは `CHECKPOINT_DB_PATH`（SQLite）へ状態保存し、中断/再開可能にする。
4. LLMの意思決定JSONは `response_mime_type="application/json"` を使用して出力安定性を担保する。
...
* Main AgentとResearch Agentは、**別コンテナ分離**を推奨する。
* Main Agentは軽量応答を維持し、重い処理はResearch Agentへジョブ委譲する。
* Main↔Research間通信は、初期は軽量HTTPまたはキュー（Redis等）でよい。どちらの場合も署名トークンで保護する。
* Eternal Explorerも同様に、Main Agentとは別コンテナ/別プロセスとして扱う。
...
* **N100/ローカルCPU環境における安定化要件（本運用設計）:**
   ...
   3. **VADの緩い有効化 (2026-04-20更新):** VADを完全無効化するとWhisperが無音チャンクにハルシネーションを生成するため、`threshold=0.3`（既定0.5より低い）で有効化し、ノイズ混じりの微小な音声も拾いつつ完全な無音は除去すること。
   ...
   5. **16kHz モノラルへの事前ダウンサンプリング:** Discord Opus デコーダの出力（48kHz ステレオ）を `main-agent` 側で 16kHz モノラルに変換してから `voice-stt-agent` へ送信すること。Whisper が内部的に期待する形式に事前変換することで認識精度を向上させる。
   6. **Whisper ハルシネーションフィルタ:** Whisper が日本語音声で頻発させる既知の幻覚パターン（「ご視聴ありがとうございました」「チャンネル登録」等）を後段フィルタで除去すること。30文字以下の短テキストのみ対象とし、実際の講義内容を誤ブロックしないこと。
...
6. **外部自動化・アクションツール [コード内実行]**
   ...
   * **※注記:** `action + parameters` の単一契約を維持し、アクションを段階的に本実装する。
...
* 未実装機能は `status=error` + `code=not_implemented_action`（または同等）を返すこと
* `stub-success` のような疑似成功レスポンスは禁止すること
* 現在、定義済みアクションに未実装はない

※実装優先モードでは、HitL安全策（承認トークン・許可コマンド制限など）を伴う範囲でCLI系ツールを先行実装してよい。

### 3.2. 内部アクション実行ツールの標準設計（追記）

内部アクション実行は、将来の拡張性と安全性を両立するため、以下の共通インターフェースを標準とする。

* **ツール名:** `execute_internal_action`
* **責務:** 外部サービス（GitHub、SMTP、将来のGoogleカレンダー等）への副作用を伴う操作を、Botコード内で直接実行する。
* **AIが生成する引数構造:**
...
* **初期アクション定義:**
   1) `add_calendar_event`
      * 必須（共通）: `title`
      * 方式A（時刻指定）: `start_time`, `end_time`
      * 方式B（終日）: `all_day=true`, `date`（`YYYY-MM-DD`）
      * 任意: `description`, `end_date`（終日の複数日、終端は包括日として扱う）
   ...
   5) `create_github_issue`
      ...
      * 実行条件: 実行環境に `GITHUB_TOKEN`（`repo` 権限）を設定していること
...
* **検証ルール:**
   * actionは許可リスト (`INTERNAL_ALLOWED_ACTIONS`) に含まれるもののみ実行。
   * 必須キーは action別スキーマ (`INTERNAL_ACTION_REQUIRED_FIELDS`) で検証。
   * `add_calendar_event` は timed と all-day の二系統を受理し、all-dayは `date` / `end_date` を優先する。
   * 日時はISO8601を推奨としつつ、運用上必要な和文日付（例: `2026年4月7日`）も受理して正規化する。
   * 検証エラー時は副作用を発生させず、ユーザーに不足項目を返す。
   * 認証未設定時は `auth_required` を返し、必要に応じて認証用URL（例: `GITHUB_AUTH_URL`）を返す。
...
* 実装優先モードでは `/action` を任意追加し、許可済みactionのみコード内で実行できるようにしてよい
* `/action` は action別の必須キー検証（payloadスキーマの最小検証）に対応できるようにする
...
* 実装優先モードでは `/action` を任意追加し、`INTERNAL_ALLOWED_ACTIONS` に含まれる action のみを実行対象とする
* `/runcli_audit` は eventフィルタ引数を受け取り、絞り込み後の末尾N件を返せるようにしてよい
* `/action` は `INTERNAL_ACTION_REQUIRED_FIELDS` に基づき action別必須キーを検証してから実行する
* `BOT_GUILD_ID` は必須設定
