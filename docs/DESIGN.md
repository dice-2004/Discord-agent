...
* **拡張性:** ルーター型エージェント（Orchestrator）を基盤とし、質問の種類に応じて適切なTool（道具）を使い分ける疎結合なアーキテクチャとします（Phase 1はGoogle公式SDK直呼び出しを許容）。
