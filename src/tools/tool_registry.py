...
class ToolRegistry:
    ...
    def _normalize_args(self, spec: ToolSpec, args: dict[str, Any]) -> tuple[dict[str, Any], str | None]:
        ...
        for key, type_desc in spec.args_schema.items():
            ...
            if key == "mode":
                allowed_modes = {"auto", "gemini_cli", "fallback"}
                mode_value = str(value).strip().lower()
                if mode_value not in allowed_modes:
                    return {}, f"modeは {sorted(allowed_modes)} のいずれかを指定してください"
                value = mode_value
            ...
            if key == "timeout_sec":
                ...
                if raw_timeout:
                    try:
                        ...
                        return {}, "timeout_sec は整数秒で指定してください"
                    if parsed < 10 or parsed > 1800:
                        return {}, "timeout_sec は 10 〜 1800 の範囲で指定してください"
                    value = str(parsed)
                else:
                    value = ""
