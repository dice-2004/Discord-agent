...
def _call_ollama_intent(text: str) -> tuple[str, float, str, str]:
    ...
    prompt = (
        ...
        "Allowed intents: add_to_jam, weather_recommend, ignore. "
        "If the utterance asks for music, intent MUST be add_to_jam. "
        "When intent is add_to_jam, extracted_query MUST be the song/artist name. "
        "CRITICAL: Keep important search context like 'anime', 'op', 'ed', 'theme song', or genre modifiers if present. "
        "CRITICAL: Strip only conversational fillers like '流して', '再生して', 'という曲', 'を入れて'. "
        "CRITICAL: Strip punctuation like '!', '?', '！', '？', '、' from the query. "
        "CRITICAL: Prefer original versions unless 'cover' is mentioned. "
        f"Utterance: {text}"
    )
...
def _process_transcript(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        with history_path.open("a", encoding="utf-8") as f:
            f.write(f"[{now_str}] Input (Whisper): 「{text}」\n")
            f.write(f"[{now_str}] Fixed (Gemma)  : 「{query}」\n")
            f.write(f"[{now_str}] Decision      : intent={intent}, action={result.get('action')}, confidence={confidence:.2f}, reason={reason}\n")
            f.write("-" * 80 + "\n")
