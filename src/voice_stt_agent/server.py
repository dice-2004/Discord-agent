...
def _call_ollama_intent(text: str) -> tuple[str, float, str, str]:
    ...
    prompt = (
        ...
        "When intent is add_to_jam, extracted_query MUST be ONLY the requested song name and artist. "
        "CRITICAL: Strip any punctuation like '!', '?', ',', '！', '？', '、' that might be noise from speech recognition. "
        "CRITICAL: Prefer original/official versions unless 'cover' or '歌ってみた' is explicitly mentioned. "
